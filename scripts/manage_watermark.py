...
def check_watermark(input_date_str, bucket_id):
    watermark_date_str = None
    
    print(f"Checking for existing watermark in {bucket_id} ...")
    try:
        # We try to download watermark.txt. Assuming the bucket is a dataset under the hood.
        # If it's a model or something else, this might need repo_type="model"
        downloaded_path = hf_hub_download(
            repo_id=bucket_id, 
            filename="watermark.txt", 
            repo_type="dataset",
            # We don't want to use cached versions from older runs if this runs locally,
            # though on GitHub actions it's always fresh.
            force_download=True 
        )
        with open(downloaded_path, "r") as f:
            watermark_date_str = f.read().strip()
...
def update_watermark(target_date_str):
    os.makedirs("data", exist_ok=True)
    with open("data/watermark.txt", "w") as f:
        f.write(target_date_str)
    print(f"Locally updated data/watermark.txt to {target_date_str}. It will be uploaded during the sync step.")
...
if __name__ == "__main__":
    ...
    if args.action == "check":
        check_watermark(args.input_date, args.bucket)
    elif args.action == "update":
        if not args.date:
            print("Error: --date is required for update action.")
            sys.exit(1)
        update_watermark(args.date)
